...
export default function CsvUploader({ onFileUpload }: CsvUploaderProps) {
  ...
  const filteredData = useMemo(() => {
    if (!processedData || !selectedSeries) return processedData;
    ...
    const totalTradeValue = filteredMatchedTrades.reduce((sum, t) => sum + t.Entry_Cost, 0);
    const weightedHoldingPeriod = totalTradeValue > 0
      ? filteredMatchedTrades.reduce((sum, t) => sum + (t.Holding_Period_Days * t.Entry_Cost / totalTradeValue), 0)
      : 0;
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (processedDataArray.length > 0) {
        ...
        if (onFileUpload) onFileUpload(combinedData);
      }
    } catch (err) {
  ...
  const clearData = () => {
    ...
    if (onFileUpload) {
      onFileUpload({
        ...
        matchedTrades: [],
        basicStats: {
          uniqueTickers: 0, totalTrades: 0, yesNoBreakdown: { Yes: 0, No: 0 },
          totalFees: 0, totalProfit: 0, avgContractPurchasePrice: 0,
          avgContractFinalPrice: 0, weightedHoldingPeriod: 0, winRate: 0, settledWinRate: 0,
        },
      });
    }
  };
  ...
  return (
    <div className="p-4">
      ...
      <div className="bg-white shadow rounded-lg p-4 mb-6">
        ...
        <p className="mb-2">To analyze your trading history, download your transaction data from Kalshi:</p>
        <ol className="list-decimal pl-6 mb-4">
          <li>Log in to your Kalshi account</li>
          <li>Go to <a href="https://kalshi.com/account/taxes" target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">Documents</a></li>
      ...
      <div className="mb-6">
        <label className="block text-sm font-medium text-gray-700 mb-2">Upload Transaction CSV Files</label>
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          {uploadedFiles.length > 0 && (
            <button onClick={clearData} className="px-4 py-2 bg-red-50 text-red-700 rounded-full text-sm font-semibold hover:bg-red-100">
              Clear Data
            </button>
          )}
        </div>
        {uploadedFiles.length > 0 && (
          <div className="mt-2">
            ...
            <ul className="list-disc pl-5 text-sm text-gray-600">
              {uploadedFiles.map((file, i) => <li key={i}>{file}</li>)}
            </ul>
          </div>
        )}
      </div>
      ...
      {filteredData && !loading && (
        <div>
          ...
          <Overview stats={filteredData.basicStats} matchedTrades={filteredData.matchedTrades} />

          <RiskAdjustedReturns matchedTrades={filteredData.matchedTrades} />
          ...
          <div className="mt-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              <TradeDirectionPie yesNoBreakdown={filteredData.basicStats.yesNoBreakdown} />
          ...
          <div className="mt-8 pt-6 border-t border-gray-200 flex justify-center">
            <a
              href="https://github.com/bnb549/kalshi-dash"
              target="_blank"
